function [Mout,Iout] = PREDECIR(Min)
% 1. Cargar modelo
load('modelo_malaria_SVM1.mat');  % Contiene mdl_svm
    ...
    nombresVariables = {'Area', 'Perimetro', 'Excentricidad', ...
    ...
    for j = 1:length(indices_wbc)
        ...
        new_row = {props.Area, props.Perimeter, props.Eccentricity, ...
    ...
    if ~isempty(caracteristicas_wbc)
        ...
        X_all = caracteristicas_wbc{:, 1:end};
    ...
    imshow(Vivax); 
   hold on;
    if ~isempty(centros_detectados)
        plot(centros_detectados(:,1), centros_detectados(:,2), 'go',  'MarkerSize', 10, 'LineWidth', 2);
    end
    frame = getframe(gca);      
    I_globulos = frame.cdata;
    end
